...
function DataGridScrollArea({
  ...
  useEffect(() => {
    ...
    if (observedElementsRef.current.header) observer?.observe(observedElementsRef.current.header)
    if (observedElementsRef.current.table) observer?.observe(observedElementsRef.current.table)
